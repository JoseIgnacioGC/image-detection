...
from src.process_model_response import process_model_response, ModelResponse
...
model_response_raw = ""
model_response = ModelResponse("Procesando imagen...", False)
...
while True:
    ...
    if not model_response_queue.empty():
        ...
        old_response_raw = model_response_raw
        model_response_raw = model_response_queue.get()

        if old_response_raw != model_response_raw:
            model_response = (
                process_model_response(
                    model_response_raw, model_response.image_description
                )
                or model_response
            )
            print(model_response)

    if not processing_img and has_one_second_passed(processing_start_time):
        processing_img = True
    ...
    display_frame = cv2.copyMakeBorder(
        frame, 0, 0, 0, 0, cv2.BORDER_CONSTANT, value=(0, 0, 0)
    )
...
# image_description = "\n".join(
#     image_description[i : i + 60] for i in range(0, len(image_description), 60)
# )
