...
def check_camera_connection():
    ...
    if not ret:
        cap.release()
        cap.open(0)
        print("Reiniciando la conexión con la cámara")
        return False
    return True
...
def update_webcam(
    ...
    ret, frame = cap.read()
    if not ret:
        print("Error al capturar frame de la cámara, intentando nuevamente...")
        return
    ...
    if has_one_second_passed(img_processing_start_time) and not is_img_processing:
        ...
        model_response_raw = '{"description": "Imagen procesada", "is_a_crime": false}'
...
def set_camera_frame(root: ctk.CTk):
    ...
    update_webcam(
        ...
        model_response=ModelResponse("", is_a_crime=False, crime_probability=0.0),  # Proporcionar crime_probability
        img_processing_start_time=datetime.now(),
